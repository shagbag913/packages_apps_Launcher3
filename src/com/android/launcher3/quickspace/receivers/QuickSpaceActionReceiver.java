...
public class QuickSpaceActionReceiver {
    ...
    private void openGoogleCalendar(View view) {
        ...
        try {
            ...
            LauncherAppsCompat.getInstance(mContext).showAppDetailsForProfile(new ComponentName(LauncherLettuceCallbacks.SEARCH_PACKAGE, ""), Process.myUserHandle());
        }
    }
    ...
    private void openGoogleWeather(View view) {
        ...
        intent.setComponent(new ComponentName(LauncherLettuceCallbacks.SEARCH_PACKAGE, "com.google.android.apps.gsa.velour.DynamicActivityTrampoline"));
        try {
            Launcher.getLauncher(mContext).startActivitySafely(view, intent, null);
        } catch (ActivityNotFoundException ex) {
            LauncherAppsCompat.getInstance(mContext).showAppDetailsForProfile(new ComponentName(LauncherLettuceCallbacks.SEARCH_PACKAGE, 
